...
if __name__ == "__main__":
	...
	if il_vs_th == 1:
		...
		audio_coeff = signal.firwin(audio_taps, audio_Fc/(audio_Fs2/2), window=('hann'))
	else:
		# to be updated by you for the takehome exercise
		# with your own code for impulse response generation
		audio_coeff = lowpass_coeff(audio_taps, audio_Fc, audio_Fs2)
	...
	if il_vs_th == 0:
		...
		audio_filt = convolve(audio_coeff,fm_demod) #np.fft.ifft(np.fft.fft(fm_demod), np.fft.fft(audio_coeff))
	...
	fmPlotPSD(ax2, audio_data, audio_Fs2/1e3, subfig_height[2], 'Downsampled Mono Audio')
