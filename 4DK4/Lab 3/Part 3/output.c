...
{
  Simulation_Run_Data_Ptr sim_data = simulation_run_data(sim_run);

  for (int channels = 1; channels <= sim_data->num_channels; channels++) {
    printf("\n%d, %f, %f", channels, sim_data->results[channels-1][0], sim_data->results[channels-1][1]);
  }
  printf("\n");
}
