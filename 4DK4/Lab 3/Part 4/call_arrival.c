...
call_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  ...
  // Make a new call for queueing or sending through
  new_call = (Call_Ptr) xmalloc(sizeof(Call));
  new_call->arrive_time = now;
  new_call->call_duration = get_call_duration_mod(simulation_run);
  //printf("Call pointer is %u\n", new_call);
  Channel_Ptr* channels = sim_data->channels;
  //for (int i=0; i<sim_data->N; i++) TRACE(printf("Channel %d is %sfree\n", i, (server_state(*(channels+i)) == FREE ? "" : "not "));)
  
  if((free_channel = get_free_channel(simulation_run)) != NULL) { // send em through
    //printf("Sending %f\n", new_call->call_duration);
    ...
    //printf("queueing %f\n", new_call->call_duration);
    TRACE(printf("Queueing, new queue size: %d\n", fifoqueue_size(sim_data->queue)+1);)
    new_call->patience = exponential_generator(sim_data->w);
