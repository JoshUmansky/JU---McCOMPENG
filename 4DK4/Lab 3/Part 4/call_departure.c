...
end_call_on_channel_event(Simulation_Run_Ptr simulation_run, void * c_ptr)
{
  ...
  Call_Ptr next_call;
  TRACE(printf("Queue size: %d\n",fifoqueue_size(sim_data->queue));)
  ...
  while(fifoqueue_size(sim_data->queue) > 0) {
    ...
    if (next_call->patience > wait_time) { // If the time since arrival is within the allowed wait time, send the call through, otherwise drop it and move on
      TRACE(printf("Putting through call\n");)
      ...
      break;
    } else { // take statistics for dropped call
      TRACE(printf("Dropped a call\n");)
      sim_data->number_of_calls_dropped += 1;
