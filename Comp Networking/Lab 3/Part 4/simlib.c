...
simulation_run_execute_event(Simulation_Run_Ptr simulation_run)
{
  ...
  /*printf("\n");
  printf("%s ", current_container->event.description);
  printf("occurring at %.3f\n", simulation_run_get_time(simulation_run));
  if(current_container->event.attachment)
    printf("%u\n", ((Channel_Ptr) (current_container->event.attachment))->customer_in_service);*/
